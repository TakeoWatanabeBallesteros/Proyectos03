...
public class ThrowKid : MonoBehaviour
{
    ...
    void Update()
    {
        if (Holding && !Input.secondaryShoot)
        {
            ...
            DEBUGCULO = ball.transform.Find("root").GetComponent<Rigidbody>();
            Debug.Log("CULOlisto");
            DEBUGCULO.AddForce(transform.forward.normalized * Fuerza, ForceMode.Impulse);
        }
